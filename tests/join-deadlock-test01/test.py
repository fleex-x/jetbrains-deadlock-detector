...
class TestJoinDeadlockClang(test_tools.MySetUpTestCase):
    def setUp(self) -> None:
        self.mySetUp(["main started", "thread started working"], "clang++")

    def test_run(self) -> None:
        error = self.launch_process()
        self.continue_to_threads_beginning(4)
        ...
        self.assertTrue(deadlock_node_cycle == [2, 3, 4, 5])


class TestJoinDeadlockCcc(test_tools.MySetUpTestCase):
    def setUp(self) -> None:
